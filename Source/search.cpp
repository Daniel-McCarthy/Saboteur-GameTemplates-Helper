...
void Search::searchTemplatesofSubtypeForKeyword(QList<GameTemplate>* templates, QString subtype, QString keyword, QTextStream* standardOut) {
    QList<QString> matches = QList<QString>();
    *standardOut << "Searching for templates containing the keyword \"" << keyword << "\":\n";
    standardOut->flush();

    for (int i = 0; i < templates->length(); i++) {
        if (templates->at(i).templateType == subtype && templates->at(i).name.contains(keyword, Qt::CaseInsensitive)) {
            matches.push_back(templates->at(i).name);
        }
    }
